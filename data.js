...
const STRINGS = {
  en: {
    ...
    assmFixNote: "* location-specific items update with the selected market",
    ...
    fixedGroups: (ltvPct) => [
      ...
      },
      {
        ...
        items: [
          "Starting capital: $100K deployed on the same date across all scenarios.",
      ...
      },
      {
        ...
        items: [
          ...
          `LTV (cash-out): new loan = max(remaining balance, ${Math.round(ltvPct * 100)}% of current home value). Any amount above the remaining balance is cash out, credited to the investor's cash position.`,
    ...
    fixedGroups: (ltvPct) => [
      ...
      },
      {
        ...
        items: [
          ...
          `LTV（套现）：新贷款 = max(剩余本金, 当前房产价值×${Math.round(ltvPct * 100)}%)，超出剩余本金部分以现金形式计入投资者持仓。`,
